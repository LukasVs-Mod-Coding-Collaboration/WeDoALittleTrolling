...
namespace WeDoALittleTrolling.Content.Projectiles
{
    ...
    public class EmbersteelShockwave : ModProjectile
    {
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            if (Projectile.GetGlobalProjectile<WDALTProjectileUtil>().TryGetParentPlayer(out Player player) && player.whoAmI == Main.myPlayer)
            {
                WDALTPlayer p = player.GetModPlayer<WDALTPlayer>();
                if (p.embersteelBladeCharges < EmbersteelBlade.maxCharges && !p.isEmbersteelExplosionActive && (!target.active || target.CanBeChasedBy()))
                {
                    p.embersteelBladeCharges++;
                    if (p.embersteelBladeCharges >= EmbersteelBlade.maxCharges)
                    {
                        for (int i = 0; i < 120; i++)
                        {
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Color backDarkColor = Color.OrangeRed; // Original Excalibur color: Color(180, 160, 60)
            Color middleMediumColor = Color.OrangeRed; // Original Excalibur color: Color(255, 255, 80)
            Color frontLightColor = Color.OrangeRed; // Original Excalibur color: Color(255, 240, 150)
