...
namespace WeDoALittleTrolling.Content.Buffs
{
    public class WreckedResistance : ModBuff
    {
        ...
        {
            tip = tip+" "+(6+(Main.player[Main.myPlayer].GetModPlayer<WDALTPlayerUtil>().wreckedResistanceStack * 6))+"%";
            base.ModifyBuffText(ref buffName, ref tip, ref rare);
        }
        
        public override bool ReApply(Player player, int time, int buffIndex)
        {
            if(player.GetModPlayer<WDALTPlayerUtil>().wreckedResistanceStack < 10)
            {
